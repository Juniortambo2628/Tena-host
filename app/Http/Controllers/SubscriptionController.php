...
class SubscriptionController extends Controller
{
    ...
    public function storePaystack(Request $request, SubscriptionService $subscriptionService)
    {
        $request->validate([
        ...
        try {
            ...
            $subscriptionService->activateForUser($user, 'paystack', $request->reference, $request->amount);
    ...
    {
        $request->validate([
            'phone_number' => 'required|regex:/^(07|01)[0-9]{8}$/',
            'amount' => 'required|numeric|min:1',
        ]);
    ...
    public function simulateMpesa(Request $request, SubscriptionService $subscriptionService)
    {
        ...
        $transaction = $subscriptionService->activateForUser($user, 'mpesa', 'SIM_'.time(), 6500);
